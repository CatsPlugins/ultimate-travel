...
if( !class_exists('cats_acf_field_cardata') ) :


class cats_acf_field_cardata extends acf_field {
	...
	*/
	...
	function __construct( $settings ) {
		...
		*/
		...
		$this->label = __('Car Data', 'ultimate-travel');
		...
		*  var message = acf._e('tourdata', 'error');
		*/
		
		$this->l10n = array(
			'error'	=> __('Error! Please enter a higher value', 'ultimate-travel'),
		);
	...
	function render_field_settings( $field ) {
		...
		*/
		...
		acf_render_field_setting( $field, array(
			'label'			=> __('Font Size','ultimate-travel'),
			'instructions'	=> __('Customise the input font size','ultimate-travel'),
			'type'			=> 'number',
			'name'			=> 'font_size',
			'prepend'		=> 'px',
		));
	...
	*/
	...
	function render_field( $field ) {
        ...
        $post = '';
        if(!is_admin() && !empty(trim(UTTTravelRequest::getQuery('post_id')))) {
            $post = get_post(trim(UTTTravelRequest::getQuery('post_id')));
        }
        UTTTravelCar::metaBoxHtml($post);
	}
	...
	*/
	...
	function validate_value( $valid, $value, $field, $input ){
		...
		if( $value < $field['custom_minimum_setting'] )
		{
			$valid = __('The value is too little!','ultimate-travel'),
		}
...
// initialize
new cats_acf_field_cardata( $this->settings );
